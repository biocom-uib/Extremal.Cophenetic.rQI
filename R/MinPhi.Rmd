...
For instance, the first 100 values of $\Phi(B_k)$ according to this recurrence are:
```{r}
Min.Phi.Rec(100)
```
...
Min.Phi(10^6)
```
and the first 100 values of $\Phi(B_k)$ computed with our closed expression are
```{r}
sapply(1:100,FUN=Min.Phi)
```
